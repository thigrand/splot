...
Aktualna wersja: **0.3.0**. Splot stosuje [Semantic Versioning](https://semver.org/lang/pl/): przed wersją 1.0 nowa widoczna funkcja podnosi drugi numer (`0.3.0`), a poprawka bez zmiany zachowania — trzeci (`0.3.1`).
...
```text
splot-materials.json
```

Na typowym profilu będzie to `Pobrane/splot-materials.json`. Rozszerzenie nie wybiera folderu za użytkownika i nie zmienia ustawień przeglądarki.

Pierwsze zatwierdzenie tworzy plik. Każde następne zatwierdzenie automatycznie zastępuje jego zawartość aktualną migawką zapisów.

## Użycie

1. Otwórz zwykłą stronę HTTP albo HTTPS i kliknij ikonę Splot.
2. Już przy otwarciu popupu powstaje wstępny rekord: pełny URL, domena, tytuł, `important` oraz `remember`. Pozostaje on tylko w pamięci rozszerzenia — nie uruchamia pobierania.
...
Zamknięcie popupu kliknięciem poza nim nie zatwierdza formularza, nie uruchamia pobierania i nie zamyka karty. Wstępny link pozostaje w pomocniczej pamięci rozszerzenia.
...
- Ręczna edycja albo usunięcie `splot-materials.json` nie jest importowana. Następny udany zapis odtwarza aktualną migawkę z pamięci rozszerzenia.
